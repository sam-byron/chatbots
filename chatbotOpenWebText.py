...
# Define a generator to process the streamed dataset
def stream_texts(dataset, max_samples=10000):
    texts = []
    for i, sample in enumerate(dataset):
        if i >= max_samples:  # Limit the number of samples
            break
        texts.append(sample["text"])
    return texts

# Process the streamed dataset
max_samples = 100000  # Adjust this value based on your memory constraints
texts = stream_texts(hf_ds, max_samples=max_samples)
print(f"Using {len(texts)} samples from the streamed dataset")

# Optimized chunk counting with batch tokenization
def count_chunks_batch(texts):
    tokenized = tokenizer(
        texts,
    ...
    return sum(len(ids) // block_size for ids in tokenized["input_ids"])

# Use multiprocessing to count chunks in parallel
chunk_size = len(texts) // num_cpu
text_batches = [texts[i:i + chunk_size] for i in range(0, len(texts), chunk_size)]

with Pool(num_cpu) as pool:
    total_chunks = sum(pool.map(count_chunks_batch, text_batches))
